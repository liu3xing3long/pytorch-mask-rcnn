...
def generate_roi(config, proposals, gt_class_ids, gt_boxes, gt_masks):
    ...
    if torch.nonzero(pos_roi_bool).size():
        ...
        pos_cnt_per_im = int(config.TRAIN_ROIS_PER_IMAGE*config.ROI_POSITIVE_RATIO)
        rand_idx = torch.randperm(pos_ind.size(0))
        ...
        DELTAS = Variable(utils.box_refinement(POS_ROIS.data, roi_gt_boxes.data), requires_grad=False)
        ...
        masks = Variable(
            CropAndResizeFunction(config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)
    ...
    if pos_cnt > 0 and neg_cnt > 0:
        ...
        zeros = Variable(torch.zeros(neg_cnt), requires_grad=False).int()
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        ROI_GT_CLASS_IDS = torch.cat([ROI_GT_CLASS_IDS, zeros], dim=0)

        zeros = Variable(torch.zeros(neg_cnt, 4), requires_grad=False)
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        DELTAS = torch.cat([DELTAS, zeros], dim=0)

        zeros = Variable(torch.zeros(neg_cnt, config.MASK_SHAPE[0], config.MASK_SHAPE[1]), requires_grad=False)
        if config.GPU_COUNT:
            zeros = zeros.cuda()
