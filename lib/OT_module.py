...
class OptTrans(nn.Module):
    def __init__(self, config, ch_x, ch_y, spatial_x=-1, spatial_y=-1,
                 epsilon=1., L=10, remove_bias=False, C_form='cosine', no_bp_P_L=True, skip_critic=False):
    ...
    def forward(self, z, y):
        """x is generated by latent variable z; y is ground truth.
                z shape (small feature):  say 15 x 1024 x 1
    ...
    def _basic_compute_loss(self, x, y):
        ...
        if self.skip_critic:
            ...
            if self.config.DEV.OT_ONE_DIM_FORM == 'fc':
                x = x.view(bs, -1)
                y = y.view(bs, -1)
        ...
        # Sinkhorn iterate
        b = Variable(torch.ones(bs, 1)*(1./bs), requires_grad=True).cuda()
        const = Variable(torch.ones(bs, 1)*(1./bs), requires_grad=False).cuda()
